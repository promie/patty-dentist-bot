...
class DentistsDidTimeslotsTidReserve(Resource):
    ...
    def put(self, did, tid):
        ...
        try:
            ...
            if final_tid not in availability:
                ...
                timeslots[dentist_name]['times'][final_tid] = 'reserved'
                write_to_file(timeslots)

                return make_response(jsonify({ 'message': f'Your appointment with <span style="color: green"><strong>{dentist_name}</strong></span> for <span style="color: green"><strong>{tid}</strong></span> has been confirmed.'}))
        except KeyError:
            return {}, 404